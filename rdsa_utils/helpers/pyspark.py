...
def count_nulls(df, subset_cols=None):
    """
    Count the number of null values in the specified columns of a PySpark df.
    ...
    if subset_cols is not None:
        ...
        assert all(
            isinstance(col, str) for col in subset_cols
...
def aggregate_col(df, col, operation):
    ...
    if operation not in valid_operations:
        raise ValueError(
            f"Invalid operation: {operation}. Must be " + f"one of {valid_operations}.",
        )
...
def drop_duplicates_reproducible(df, col, id_col=None):
    """
...
def apply_col_func(df, cols, func):
    ...
    # Assert that cols is a list
    assert isinstance(cols, list), "cols must be a list."

    # Assert that all elements in cols are strings
    assert all(isinstance(col, str) for col in cols), (
        "All elements in cols must be strings.")
...
def pyspark_random_uniform(df, output_colname, lower_bound=0, upper_bound=1, seed=None):
    """
    Mimic numpy.random.uniform for PySpark.

    Creates a column with random numbers between specified values
    'lower_bound' and 'upper_bound', drawn from a uniform distribution.
    ...
    assert isinstance(lower_bound, (int, float)), "lower_bound must be a number."
    assert isinstance(upper_bound, (int, float)), "upper_bound must be a number."
    assert lower_bound < upper_bound, "lower_bound must be less than upper_bound."
...
def sum_columns(df, cols_to_sum, output_col):
    ...
    assert all(
        isinstance(col, str) for col in cols_to_sum
...
def set_nulls(df, column, values):
    ...
    if isinstance(values, str):
        values = [values]
    assert isinstance(values, list), "values must be a list of strings or a string."
    assert all(
...
def union_multi(df_list):
    ...
    Note: All SparkDFs must have the same columns.
    ...
    assert all(
        isinstance(df, SparkDF) for df in df_list
...
def join_multi(df_list, on, how):
    ...
    # check 'on'
    assert isinstance(on, (str, list)), "'on' must be a string or a list of strings"

    if isinstance(on, list) and not all(isinstance(col, str) for col in on):
        error_message = "All elements in 'on' must be strings"
        raise ValueError(error_message)
